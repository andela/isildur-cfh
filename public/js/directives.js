...
.directive('player', () => ({
  ...
  link: (scope, elem, attr) => {
    scope.colors = ['#7CE4E8',
      '#FFFFa5', '#FC575E', '#F2ADFF', '#398EC4', '#8CFF95'];
  }
}))
.directive('answers', () => ({
  restrict: 'EA',
  ...
  scope.colors[scope.game.players[scope.game.winningCardPlayer].color]
        ...
        const removePunctuation = (cardIndex) => {
          let cardText = scope.game.table[scope.game.winningCard]
            .card[cardIndex].text;
          if (cardText.indexOf('.',
            cardText.length - 2) === cardText.length - 1) {
            cardText = cardText.slice(0, cardText.length - 1);
          } else if ((cardText.indexOf('!',
            cardText.length - 2) === cardText.length - 1 ||
              cardText.indexOf('?',
                cardText.length - 2) === cardText.length - 1) &&
        ...
        if (curQuestionArr.length > 1) {
          ...
          if (shouldRemoveQuestionPunctuation) {
            if (curQ.text.indexOf('.',
              curQ.text.length - 2) === curQ.text.length - 1) {
